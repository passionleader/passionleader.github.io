...
```sql
SELECT buyTbl.userID, buyTbl.prodName, userTbl.name, userTbl.addr, CONCAT(mobile1,mobile2) AS '휴대폰번호'
FROM buyTbl
INNER JOIN userTbl
...
```SQL
SELECT B.userID, B.prodName, U.name, U.addr, CONCAT(mobile1,mobile2) AS '휴대폰번호'
FROM buyTbl B
